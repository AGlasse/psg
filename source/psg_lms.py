...
class PSG_lms:
    ...
    def configure(**kwargs):
        PSG_lms.dit = kwargs.get('dit', 100.)
        PSG_lms.pce = kwargs.get('pce', 0.1)
        PSG_lms.rnoise = kwargs.get('rnoise', 10.)
        PSG_lms.n_pix_aper = kwargs.get('n_pix_aper', 6)
        return
    ...
    def detect(self, flux_spectrum, **kwargs):
        ...
        area_elt = math.pi * math.pow(PSG_lms.d_elt / 2., 2.)
        ...
        bw[-1] = bw[-2]
        n_pix = PSG_lms.n_pix_aper
        sa_pix = n_pix * solid_angle_pix
        rad_pix = flux * area_elt * bw * sa_pix             # ph/sec/pixel
        sig_pix = rad_pix * PSG_lms.pce * PSG_lms.dit       # el/pixel
        noise_pix = np.sqrt(sig_pix) + PSG_lms.rnoise
