...
class PSG_sky:
    ...
    def load_spectra(self):
        ...
        trans_units = '-'
        PSG_sky.trans_spectrum = Spectrum(wave, trans, None, '-', 'Transmission', 'black')
        flux = data_table['flux']
        flux_units = 'ph/s/m2/um/arcsec2'
        PSG_sky.flux_spectrum = Spectrum(wave, flux, None, flux_units, 'Flux', 'blue')
        print("Loaded sky transmission and emission spectrum with units {:s}".format(flux_units))
        return

    def get_spectrum(self, ord):
        if ord == 'flux':
            return PSG_sky.flux_spectrum
        else:
            return PSG_sky.trans_spectrum
